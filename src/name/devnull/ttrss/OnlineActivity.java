...
import name.devnull.ttrss.offline.OfflineActivity;
import name.devnull.ttrss.offline.OfflineDownloadService;
import name.devnull.ttrss.offline.OfflineUploadService;
import name.devnull.ttrss.share.SubscribeActivity;
import name.devnull.ttrss.types.Article;
import name.devnull.ttrss.types.ArticleList;
import name.devnull.ttrss.types.Feed;
import name.devnull.ttrss.types.Label;
...
//		SharedPreferences localPrefs = getSharedPreferences("localprefs", Context.MODE_PRIVATE);
	...
	public void checkTrial(boolean notify) {
		boolean isTrial = getPackageManager().checkSignatures(
				getPackageName(), "name.devnull.ttrss.key") != PackageManager.SIGNATURE_MATCH;
	...
	private void openUnlockUrl() {
		try {
			Intent intent = new Intent(Intent.ACTION_VIEW, 
				Uri.parse("market://details?id=name.devnull.ttrss.key"));
			startActivity(intent);
		} catch (ActivityNotFoundException ae) {
			try {
				Intent intent = new Intent(Intent.ACTION_VIEW, 
					Uri.parse("https://play.google.com/store/apps/details?id=name.devnull.ttrss.key"));
				startActivity(intent);
			} catch (Exception e) {
				e.printStackTrace();
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		...
		for (PackageInfo p : pkgs) {
			if ("name.devnull.ttrss.key".equals(p.packageName)) {
